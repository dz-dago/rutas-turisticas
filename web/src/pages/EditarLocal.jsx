...
function EditarLocal() {
  ...
  useEffect(() => {
    ...
    const localGuardado =
      JSON.parse(
        localStorage.getItem("localEditar")
      );

    if (!localGuardado) return;

    setForm({
      nombre: localGuardado.nombre || "",
      categoria: localGuardado.categoria || "GAS",
      descripcion: localGuardado.descripcion || "",
      direccion:
        localGuardado.ubicacion?.direccion || "",
      lat:
        localGuardado.ubicacion?.lat || "",
      lng:
        localGuardado.ubicacion?.lng || "",
      linkMaps: "",
      precioPromedio:
        localGuardado.precioPromedio || "",
      foto:
        localGuardado.fotos?.[0] || "",
      urlFoto:
        localGuardado.fotos?.[0] || ""
    });
  ...
  const actualizarLocal = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.mensaje || "Error al actualizar local");
    } finally {
      setGuardando(false);
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen bg-zinc-950 text-white px-5 md:px-8 py-10">
        <h1 className="text-4xl font-black">Editar Local</h1>
        ...
        >
          ...
          >
            {guardando ? "Guardando..." : "Actualizar local"}
          </button>
        </form>
      </div>
    </DashboardLayout>
